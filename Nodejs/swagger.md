...
swagger-output.json을 미리 만들어둬야 함.

우선 app.js에 기본적인 서버구조를 만들어주겠습니다.ㄷ
...
이러면 이런 swagger-output.json파일이 만들어집니다. 이제 /api-docs에 접근해서 문서를 확인할 수 있습니다.
다만 swagger-autogen은 기본적으로 API경로와 status code정도만 분석하기 때문에, API 설명, requestBody, reponseBody에 대한 정보들은
수작업으로 넣어줘야 합니다. 마지막으로 그 작업을 하고 끝내겠습니다.
