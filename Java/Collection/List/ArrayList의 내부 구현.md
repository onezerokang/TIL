...
일반적인 배열은 한번 크기가 정해지면 그 크기를 변경할 수 없다. 따라서, 정해진 크기를 초과하여 데이터를 추가하려고 하면 `ArrayIndexOutOfBoundsException`이 발생한다. 이런 제한으로 인해 배열은 사용에 제약이 발생한다.
...
현재 배열의 용량이 꽉 찼다면 grow() 메서드를 호출하여 배열을 키우고 요소를 추가한다.
