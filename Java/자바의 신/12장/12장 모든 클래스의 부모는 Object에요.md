...
Object 메소드를 정의하고 이를 상속하게 함으로써 모든 클래스가 가져야 할 최소한의 기능을 설정하는 것이다.
...
- public int hashCode()
  - 객체에 대한 해시 코드 값을 리턴한다. 해시 코드는 16진수로 제공되는 객체의 고유 값을 말한다.
...
Object의 메소드만으로는 해당 객체의 실제 속성 값을 알 수 없기 때문에 toString 메소드를 사용할 것이라면 오버라이딩을 구현해야 한다.
...
hashCode() 메소드는 객체의 고유 값을 리턴하는데, 만약 해당 메소드를 오버라이딩 하지 않았다면, 기본적으로 객체의 메모리 주소값을 반환하도록 정의되어있다.

자바에는 두 객체의 equals() 값이 true일 때 hashCode() 값은 무조건 동일 해야 한다는 규칙이 있기에, equals()를 오버라이딩하면 hashCode 메소드도 오버라이딩하여 동일한 값이 나오도록 해야 한다.
